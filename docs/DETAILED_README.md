# PartSelect AI Chat Agent - Detailed Implementation Notes

This document is the long-form implementation README. The project root README is now the visual quick-start page for reviewers and demo viewers; this file keeps the deeper architecture, implementation choices, operational commands, and next-stage roadmap.

## Executive summary

The app is a PartSelect-style AI shopping assistant for refrigerator and dishwasher parts. It combines a React chat widget, FastAPI backend, deterministic handlers for high-confidence e-commerce flows, PostgreSQL catalog/session/cart storage, pgvector troubleshooting retrieval, and optional live scraping for missing catalog coverage.

The core product decision is **deterministic-first routing**. The system uses an LLM to classify intent, then routes most business actions to explicit handlers instead of asking an agent to improvise. This keeps demo flows repeatable:

- Part lookup, compatibility checks, installation instructions, and cart actions are deterministic.
- Troubleshooting uses RAG plus synthesis because symptom explanations need natural language.
- LangGraph remains available only for general fallback turns.
- Runtime scraping is a fallback, not the desired long-term primary data source.

## Visual system overview

```mermaid
flowchart LR
    User[Customer] --> Widget[React chat widget]
    Widget --> SSE[SSE progress stream]
    SSE --> ChatAPI[FastAPI chat router]
    ChatAPI --> Guardrail[Scope guardrail]
    Guardrail --> Classifier[Structured LLM classifier]
    Classifier --> Intent{Intent}

    Intent -->|search / parts_for_model| Catalog[Catalog resolver]
    Intent -->|compatibility| Compat[Compatibility handler]
    Intent -->|install| Install[Installation handler]
    Intent -->|cart| Cart[Cart handler]
    Intent -->|troubleshoot| RAG[Repair RAG handler]
    Intent -->|general| Agent[LangGraph fallback]

    Catalog --> DB[(PostgreSQL)]
    Compat --> DB
    Install --> DB
    Cart --> Session[(Session + cart tables)]
    RAG --> Vector[(pgvector embeddings)]
    RAG --> Synth[Synthesis LLM]
    Catalog --> Live[Optional live scrape]
    Live --> PartSelect[PartSelect pages]

    DB --> Response[Structured response metadata]
    Session --> Response
    Vector --> Synth --> Response
    Response --> Widget
```
...
sequenceDiagram
    ...
    participant LS as Live scrape fallback

    U->>W: Sends message
    W->>API: POST /api/chat stream=true
    API-->>W: stage: Understanding your request
    API->>R: Scope check + classify intent
    R-->>API: intent + slots
    API-->>W: stage: Checking matching data
    API->>H: Run deterministic handler or RAG path
    H->>DB: Query catalog, compatibility, session, cart, or embeddings
    alt DB is sparse and live scrape is enabled
        H->>LS: Fetch model or part page
        LS-->>H: Ephemeral scraped result
    end
    H-->>API: text + parts + metadata
    API-->>W: done event
    W-->>U: Rich answer, cards, cart update
```

## Data and scraping strategy

```mermaid
flowchart TB
    Curated[Curated URLs] --> Bulk[Bulk Selenium collection]
    Bulk --> Details[Product detail rows]
    Bulk --> Repairs[Repair guides]
    Bulk --> Articles[How-to articles]
    Details --> Merge[Incremental merge]
    Repairs --> Raw[backend/data/raw/*.jsonl]
    Articles --> Raw
    Merge --> Raw
    Raw --> Ingest[Ingest + embedding generation]
    Ingest --> Parts[(parts)]
    Ingest --> Compat[(compatibility)]
    Ingest --> Docs[(repair_guides + articles)]
    Ingest --> Embeddings[(pgvector embeddings)]

    Runtime[Chat runtime] --> Parts
    Runtime --> Compat
    Runtime --> Embeddings
    Runtime --> LiveFallback[Live scrape fallback for missing/sparse records]
```

Long term, the preferred data model is a **nightly whole-catalog scrape and refresh**. Live scrape should become an exception path for missing rows, validation, and emergency freshness rather than a per-query dependency.

## Next-stage roadmap

### 1. Frontend and UX polish

- Make the widget feel closer to a production PartSelect experience: better empty states, tighter product-card hierarchy, clearer source badges, and stronger mobile ergonomics.
- Add explicit visual states for `source=db`, `source=live`, model mismatch, no part-type match, and out-of-scope responses.
- Surface troubleshooting-related parts more prominently, with clearer separation between diagnostic advice and purchasable parts.
- Add richer cart affordances: quantity stepper, subtotal persistence, and stronger confirmation after contextual commands like "add it".
- Improve the embeddable mode documentation with a tiny host-page example and tested build asset instructions.

### 2. Nightly full scrape instead of live scrape per query

- Run the scraper as a scheduled nightly job that crawls product details, compatibility cross-references, repair guides, and articles.
- Store scrape checkpoints and run summaries so failed stages can resume cleanly.
- Merge changed rows into JSONL or a staging database, validate row counts and required fields, then promote to the runtime database.
- Treat runtime live scraping as an audit/fallback path only, controlled by config and logged clearly.
- Add completeness dashboards: models covered, parts per model, products with images, products with prices, products with installation steps, and repair-guide embedding counts.

### 3. Redis caching

- Use Redis for short-lived cache entries that reduce repeated expensive work:
  - classified intent for identical messages in the same session shape
  - model catalog lookups
  - part detail lookups
  - compatibility checks
  - live scrape responses
  - RAG retrieval result IDs
- Keep cart/session source of truth in Postgres; Redis should accelerate reads and expensive external calls, not own durable user state.
- Add TTLs by data type. Compatibility and catalog rows can live longer; live scrape and LLM-derived classifications should be shorter.
- Include cache hit/miss fields in Loguru event logs.

### 4. Production hardening

- Expand scenario tests around pending slots, pronoun cart resolution, model mismatch, and unavailable live scrape.
- Add contract tests for response metadata consumed by the frontend.
- Add rate limits and request-size limits around `/api/chat`.
- Add structured health checks for DB, Redis, embedding availability, and LLM provider availability.
- Make observability dashboards from existing event names: intent latency, handler latency, source mix, live scrape failures, and cart conversion events.
